    ...
    methods (Access = protected)
        ...
        function answer = PositionList(obj)
            ...
            for ii = 1:numel(app.specObj)
                answer.positionList(ii) = struct('IDN',       app.specObj(ii).ReceiverId,                 ...
                                                 'gpsType',   app.specObj(ii).TaskSpec.Script.GPS.Type, ...
                                                 'gpsStatus', app.specObj(ii).GPSLastFix.Status,       ...
                                                 'Latitude',  app.specObj(ii).GPSLastFix.Latitude,     ...
                                                 'Longitude', app.specObj(ii).GPSLastFix.Longitude);
            end
        end
        ...
        function answer = TaskList(obj)            
            ...
            for ii = 1:numel(app.specObj)
                answer.taskList(ii).IDN          = app.specObj(ii).ReceiverId;
                answer.taskList(ii).TaskName     = app.specObj(ii).TaskSpec.Script.Name;
                answer.taskList(ii).Observation  = struct('Type',      app.specObj(ii).TaskSpec.Script.Observation.Type, ...
                                                          'BeginTime', app.specObj(ii).Timing.startedAt,        ...
                                                          'EndTime',   app.specObj(ii).Timing.endedAt);
                
                maskTable = [];
                for jj = 1:numel(app.specObj(ii).Bands)
                    Mask = app.specObj(ii).Bands(jj).Mask;
                    if ~isempty(Mask)
                        maskTable = Mask.Table;
                        Mask = rmfield(Mask, {'Table', 'Array', 'BrokenArray'});
                    end

                    answer.taskList(ii).Band(jj) = struct('FreqStart',          app.specObj(ii).TaskSpec.Script.Band(jj).FreqStart,                ...
                                                          'FreqStop',           app.specObj(ii).TaskSpec.Script.Band(jj).FreqStop,                 ...
                                                          'ObservationSamples', app.specObj(ii).TaskSpec.Script.Band(jj).instrObservationSamples,  ...
                                                          'nSweeps',            app.specObj(ii).Bands(jj).nSweeps,                              ...
                                                          'Mask',               Mask);
                end

                if ~strcmp(app.specObj(ii).TaskSpec.Script.Observation.Type, 'Samples')
